<div class="common-sec">
            ...
            <div class="overflow-hidden bg-white">
                <div class="">
                    ...
                    <div class="w-100 gap d-flex sm:justify-between justify-center items-end">
                        ...
                        <form action="{{ route('unpublish.level2.result') }}" method="POST">
                            @csrf
                            <button type="submit" class="table-btn green">
                                <span>Unpublish Result Level 2</span>
                            </button>
                        </form>
                        <!-- Publish Button -->
                        ...
                        <form action="{{ route('publish.level2.result') }}" method="POST">
                            @csrf
                            <button type="submit" class="table-btn red">
                                <span>Publish Result Level 2</span>
                            </button>
                        </form>
                        ...
                        <table class="min-w-full border divide-y divide-gray-200">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td align="center">
                                        ...
                                        <div id="prototype-files{{$admin->id}}" style="display:none;max-width:700px;">
                                            <div class="heading short text-center mb-6">Prototype File</div>
                                            <ul>
                                                <li>
                                                    <strong>Title </strong>
                                                    <p>
                                    ...
                                    <td align="center">
                                        ...
                                        <button data-fancybox data-src="#dialog-contentss" type="button" class="table-btn green approve-button" data-team-id="{{ $teamName->id }}" data-college-id="{{ $teamName->college_id }}" onclick="showModal({{ $teamName->id }}, {{ $teamName->college_id }}, true)">
                                            Select
                                        </button>
                                        @elseif($teamName->approved_level2 == 0)
                                        <button data-fancybox data-src="#dialog-contentss" type="button" class="table-btn red approve-button" data-team-id="{{ $teamName->id }}" data-college-id="{{ $teamName->college_id }}" onclick="showModal({{ $teamName->id }}, {{ $teamName->college_id }}, false)">
                                            Not Select
                                        </button>
                                        @else
                                        <button data-fancybox data-src="#dialog-contentss" type="button" class="table-btn  approve-button" data-team-id="{{ $teamName->id }}" data-college-id="{{ $teamName->college_id }}" onclick="showModal({{ $teamName->id }}, {{ $teamName->college_id }}, false)">
                                            Select Team
                                        </button>
                                        @endif
                                    </td>
                                ...
                                @if(isset($teamName->approved_level2))
                                                                            ...
                                                                            @else
                                                                                <button type="button" class="table-btn red">
    ...
    <div id="dialog-contentss" style="display: none;">
        <div class="text-center">
            <p><b>Select Team for Level 3</b></p>
            <div class="buttons-sec d-flex justify-center w-100 links mt-6">
                <button type="button" class="common-btn admin-btn green" id="confirmApproveButton" onclick="approveTeam()">Select</button>
                <button type="button" class="common-btn admin-btn red" id="notconfirmApproveButton" onclick="notApproveTeam()">Reject</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function notApproveTeam() {
            fetch(`/notapprove-team/${selectedTeamId}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
